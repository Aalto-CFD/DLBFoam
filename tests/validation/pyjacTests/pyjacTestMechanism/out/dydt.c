...
void dydt (const double t, const double pres, const double * __restrict__ y, double * __restrict__ dy) {
  ...
  dy[0] = (-1.0 / (rho * cp_avg)) * ((dy[1] * h[0] * 1.6042999999999999e+01)
        + (dy[2] * h[1] * 3.0026000000000000e+01) + (dy[3] * h[2] * 3.1033999999999999e+01)
        + (dy[4] * h[3] * 1.0080000000000000e+00) + (dy[5] * h[4] * 3.1998000000000001e+01)
        + (dy[6] * h[5] * 2.0160000000000000e+00) + (dy[7] * h[6] * 1.5999000000000001e+01)
        + (dy[8] * h[7] * 1.7007000000000001e+01) + (dy[9] * h[8] * 1.8015000000000001e+01)
        + (dy[10] * h[9] * 3.3006000000000000e+01) + (dy[11] * h[10] * 3.4014000000000003e+01)
        + (dy[12] * h[11] * 1.2010999999999999e+01) + (dy[13] * h[12] * 1.3018999999999998e+01)
        + (dy[14] * h[13] * 1.4026999999999999e+01) + (dy[15] * h[14] * 1.4026999999999999e+01)
        + (dy[16] * h[15] * 1.5035000000000000e+01) + (dy[17] * h[16] * 2.8009999999999998e+01)
        + (dy[18] * h[17] * 4.4009000000000000e+01) + (dy[19] * h[18] * 2.9018000000000001e+01)
        ...
        + (dy[49] * h[48] * 4.3088999999999999e+01) + (dy[50] * h[49] * 4.4097000000000001e+01)
        + (dy[51] * h[50] * 4.3045000000000002e+01) + (dy[52] * h[51] * 4.4052999999999997e+01)
        + (dy_N * h[52] * 2.8013999999999999e+01));

  // calculate rate of change of species mass fractions
  dy[1] *= (1.6042999999999999e+01 / rho);
  dy[2] *= (3.0026000000000000e+01 / rho);
  dy[3] *= (3.1033999999999999e+01 / rho);
  dy[4] *= (1.0080000000000000e+00 / rho);
  dy[5] *= (3.1998000000000001e+01 / rho);
  dy[6] *= (2.0160000000000000e+00 / rho);
  dy[7] *= (1.5999000000000001e+01 / rho);
  dy[8] *= (1.7007000000000001e+01 / rho);
  dy[9] *= (1.8015000000000001e+01 / rho);
  dy[10] *= (3.3006000000000000e+01 / rho);
  dy[11] *= (3.4014000000000003e+01 / rho);
  dy[12] *= (1.2010999999999999e+01 / rho);
  dy[13] *= (1.3018999999999998e+01 / rho);
  dy[14] *= (1.4026999999999999e+01 / rho);
  dy[15] *= (1.4026999999999999e+01 / rho);
  dy[16] *= (1.5035000000000000e+01 / rho);
  dy[17] *= (2.8009999999999998e+01 / rho);
  dy[18] *= (4.4009000000000000e+01 / rho);
  dy[19] *= (2.9018000000000001e+01 / rho);
  ...
  dy[48] *= (3.9950000000000003e+01 / rho);
  dy[49] *= (4.3088999999999999e+01 / rho);
  dy[50] *= (4.4097000000000001e+01 / rho);
  dy[51] *= (4.3045000000000002e+01 / rho);
  dy[52] *= (4.4052999999999997e+01 / rho);
...
void dydt (const double t, const double rho, const double * __restrict__ y, double * __restrict__ dy) {
  ...
  // rate of change of temperature
  dy[0] = (-1.0 / (rho * cv_avg)) * ((dy[1] * u[0] * 1.6042999999999999e+01)
        + (dy[2] * u[1] * 3.0026000000000000e+01) + (dy[3] * u[2] * 3.1033999999999999e+01)
        + (dy[4] * u[3] * 1.0080000000000000e+00) + (dy[5] * u[4] * 3.1998000000000001e+01)
        + (dy[6] * u[5] * 2.0160000000000000e+00) + (dy[7] * u[6] * 1.5999000000000001e+01)
        + (dy[8] * u[7] * 1.7007000000000001e+01) + (dy[9] * u[8] * 1.8015000000000001e+01)
        + (dy[10] * u[9] * 3.3006000000000000e+01) + (dy[11] * u[10] * 3.4014000000000003e+01)
        + (dy[12] * u[11] * 1.2010999999999999e+01) + (dy[13] * u[12] * 1.3018999999999998e+01)
        + (dy[14] * u[13] * 1.4026999999999999e+01) + (dy[15] * u[14] * 1.4026999999999999e+01)
        + (dy[16] * u[15] * 1.5035000000000000e+01) + (dy[17] * u[16] * 2.8009999999999998e+01)
        + (dy[18] * u[17] * 4.4009000000000000e+01) + (dy[19] * u[18] * 2.9018000000000001e+01)
        ...
        + (dy[49] * u[48] * 4.3088999999999999e+01) + (dy[50] * u[49] * 4.4097000000000001e+01)
        + (dy[51] * u[50] * 4.3045000000000002e+01) + (dy[52] * u[51] * 4.4052999999999997e+01)
        + (dy_N * u[52] * 2.8013999999999999e+01));

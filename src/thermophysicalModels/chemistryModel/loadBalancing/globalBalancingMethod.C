...
void globalBalancingMethod::update_state(
...
globalBalancingMethod::operations_to_info(
...
std::vector<label> globalBalancingMethod::times_to_problem_counts(
...
globalBalancingMethod::get_operations(DynamicList<chemistryLoad>& loads,
                                      const chemistryLoad&        my_load)
{
    ...
    while(sender != receiver) {
        ...
        double    send_value= std::min(sender->value - global_mean,
                                    global_mean - receiver->value);
    ...
    runtime_assert(!((is_sender(operations, my_load.rank)
                      && is_receiver(operations, my_load.rank))),
                   "Only sender or receiver should be possible.");

    runtime_assert(std::abs(get_mean(loads) - global_mean) < 1E-7,
                   "Vanishing load");
...
bool globalBalancingMethod::is_sender(const std::vector<Operation>& operations,
                                      int                           rank)
{
...
bool globalBalancingMethod::is_receiver(

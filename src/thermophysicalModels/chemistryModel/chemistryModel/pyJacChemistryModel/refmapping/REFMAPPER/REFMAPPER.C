...
Foam::chemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::REFMAPPER
(
    ...
    //cleaningRequired_(false)
{
    ...
    */
    ...
    if (this->log())
    {
        nRetrievedFile_ = chemistry.logFile("found_REFMAPPER.out");
        nGrowthFile_ = chemistry.logFile("growth_REFMAPPER.out");
        nAddFile_ = chemistry.logFile("add_REFMAPPER.out");
        sizeFile_ = chemistry.logFile("size_REFMAPPER.out");
    }
    */
}
...
/*
template<class CompType, class ThermoType>
void Foam::chemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::addToMRU
(
    chemPointREFMAPPER<CompType, ThermoType>* phi0
)
{
    ...
    if (maxMRUSize_ > 0 && MRURetrieve_)
    {
        ...
        bool isInList = false;
        typename SLList <chemPointREFMAPPER<CompType, ThermoType>*>::iterator iter =
            MRUList_.begin();
...
template<class CompType, class ThermoType>
void Foam::ChemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::calcNewC
(
    chemPointREFMAPPER<CompType, ThermoType>* phi0,
...
template<class CompType, class ThermoType>
bool Foam::ChemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::grow
(
    chemPointREFMAPPER<CompType, ThermoType>* phi0,
    const scalarField& phiq,
...
)
{
    ...
    // The actual solution and the approximation given by REFMAPPER are too different
    else
    {
        return false;
    }
...
template<class CompType, class ThermoType>
bool
Foam::ChemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::cleanAndBalance()
{
    ...
    chemPointREFMAPPER<CompType, ThermoType>* x = chemisTree_.treeMin();
    while(x != nullptr)
    {
        chemPointREFMAPPER<CompType, ThermoType>* xtmp =
...
template<class CompType, class ThermoType>
void Foam::ChemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::computeA
(
...
/*
template<class CompType, class ThermoType>
bool Foam::ChemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::retrieve
(
...
)
{
    ...
    if (chemisTree_.size())
    {
        ...
        else if (MRURetrieve_)
        {
            typename SLList
            <
                chemPointREFMAPPER<CompType, ThermoType>*
...
template<class CompType, class ThermoType>
Foam::label Foam::ChemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::add
(
    const scalarField& phiq,
    const scalarField& Rphiq,
    const scalar rho,
...
)
{
    ...
    if (chemisTree().isFull())
    {
        ...
        if (!cleanAndBalance())
        {
            DynamicList<chemPointREFMAPPER<CompType, ThermoType>*> tempList;
            if (maxMRUSize_>0)
            {
                // Create a copy of each chemPointREFMAPPER of the MRUList_ before
                // they are deleted
                typename SLList
                <
                    chemPointREFMAPPER<CompType, ThermoType>*
                ...
                for ( ; iter != MRUList_.end(); ++iter)
                {
                    tempList.append
                    (
                        new chemPointREFMAPPER<CompType, ThermoType>(*iter())
                    );
                }
            }
            chemisTree().clear();
            ...
            chemPointREFMAPPER<CompType, ThermoType>* nulPhi = 0;
            forAll(tempList, i)
            {
...
template<class CompType, class ThermoType>
void
Foam::ChemistryRefMappingMethods::REFMAPPER<CompType, ThermoType>::writePerformance()
{

...
To attempt to install to the core OpenFOAM installation (```$FOAM_LIBBIN```) (Use with caution as it overwrites dynamic code dictionaries!), use:
```
WM_BUILD=core ./Allwmake --platform <LAPACK_INSTALLATION_TYPE>
```
...
[ct2foam](https://github.com/kahilah/ct2foam) makes this process easy and enables you to convert and re-fit chemical mechanism thermophysical data into OpenFOAM format. Please check ```utilities/pyjac2foam.sh``` for required steps on compiling pyJac and calling ct2foam accordingly.
...
* Select type ```loadBalanced_pyJac``` in ```constant/chemistryProperties```:

```
type    loadBalanced_pyJac;
```
...
- Heikki Kahila (k4h114h@proton.me)

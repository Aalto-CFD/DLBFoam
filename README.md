...
## Build / Test control flags

The Allwmake script supports two flags to separate build and test phases:

- `--skip-tests` — build only; do not run bundled validation/unit tests.
  Example (build only, useful for CI build step):

```
./Allwmake --platform STANDALONE --skip-tests
```

- `--only-tests` — run only the test suite, assuming build artifacts are already present.
Example (CI test step after a successful build):

```
./Allwmake --only-tests
```

Use these flags to split compilation and testing in CI (build step: `--skip-tests`, test step: `--only-tests`) so failures are reported separately and iteration is faster.
